...
interface Converter<From : Any, To : Any> {
     ...
     * @param listener The error handler to use when the conversion fails. Defaults to [ThrowOnError] if not provided.
     * @return Returns [To] if the conversion succeeded. If the conversion failed, returns null unless the error handler
     * throws an exception
     * @throws ProblemException Thrown by error handler if the conversion fails and the listener is [ThrowOnError]
     *
     * @see ProblemListener
     * @see ReturnOnError
     * @see ThrowOnError
     */
    fun convert(from: From?, listener: ProblemListener = throwOnError): To?
